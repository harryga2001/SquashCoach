...
export default function App() {
  ...
  return (
    <><style>{css}</style>
    <Nav name={profile.name} email={session.email} role={role} onLogout={logout} />
    <div className="wrap">
      {isAdmin && <AdminPanel token={session.token} />}
      {!isAdmin && role === "coach" && <CoachDash coach={profile} token={session.token} />}
