...
// ─── SUPABASE ─────────────────────────────────────────────────────────────────
const URL = "https://ntninbfowpvoyifstbqx.supabase.co/rest/v1";
const KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im50bmluYmZvd3B2b3lpZnN0YnF4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA5MTY1NTUsImV4cCI6MjA5NjQ5MjU1NX0.ZCTjtzBBTtFp46uDr320R-hq2fQBRl5MUwC5m4LaCpA";
const H = { "apikey": KEY, "Authorization": `Bearer ${KEY}`, "Content-Type": "application/json" };

async function dbGet(table, query = "") {
  const r = await fetch(`${URL}/${table}?${query}`, { headers: H });
  if (!r.ok) { const e = await r.json(); throw new Error(e.message || r.status); }
  return r.json();
}
async function dbInsert(table, body) {
  const r = await fetch(`${URL}/${table}`, { method: "POST", headers: { ...H, "Prefer": "return=representation" }, body: JSON.stringify(body) });
  if (!r.ok) { const e = await r.json(); throw new Error(e.message || r.status); }
  const d = await r.json(); return Array.isArray(d) ? d[0] : d;
}
async function dbUpdate(table, query, body) {
  const r = await fetch(`${URL}/${table}?${query}`, { method: "PATCH", headers: { ...H, "Prefer": "return=representation" }, body: JSON.stringify(body) });
  if (!r.ok) { const e = await r.json(); throw new Error(e.message || r.status); }
  return r.json();
}

// ─── ADMIN EMAIL ──────────────────────────────────────────────────────────────
const ADMIN = "harry.g.a2001@gmail.com";
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .auth-hint { margin-top: 24px; padding-top: 20px; border-top: 1px solid var(--border); font-size: 11px; color: var(--dim); line-height: 1.9; }
  ...
  .card-name { font-family: 'Barlow Condensed', sans-serif; font-size: 19px; font-weight: 800; letter-spacing: 0.02em; }
  ...
  .card-week { font-size: 11px; color: var(--acc); font-weight: 700; letter-spacing: 0.04em; }
  ...
  .dot-Squash { background: var(--squash); } .dot-Gym { background: var(--gym); } .dot-Rest { background: var(--rest); }
  ...
  .tp-Gym { background: rgba(0,200,255,0.1); color: var(--gym); }
  ...
  .spin { display: inline-block; width: 16px; height: 16px; border: 2px solid var(--border); border-top-color: var(--acc); border-radius: 50%; animation: spin 0.6s linear infinite; margin-right: 8px; vertical-align: middle; }
...
function AuthScreen({ onLogin }) {
  ...
  const signup = async () => {
    ...
    setBusy(true); setErr("");
    try {
      await dbInsert("coaches", { name: f.name.trim(), email: f.email.trim().toLowerCase(), password: f.password, bio: f.bio.trim(), status: "pending" });
      setOk("Application submitted! You'll get access once approved.");
      setF({ email: "", password: "", name: "", bio: "" });
    } catch (e) { setErr("That email may already be registered."); }
    setBusy(false);
  };
    ...
    <div className="auth">
      <div className="auth-box">
        ...
        <div className="auth-tabs">
          <button className={`auth-tab ${tab === "login" ? "on" : ""}`} onClick={() => { setTab("login"); setErr(""); setOk(""); }}>Sign In</button>
        ...
        {tab === "signup" && !ok && (
          <>
            ...
            <div className="fld"><label className="lbl">Password</label><input className="inp" type="password" placeholder="Choose a password" value={f.password} onChange={e => s("password", e.target.value)} /></div>
        ...
        <div className="auth-hint">
          <b>Coach login:</b> harry.g.a2001@gmail.com / Squash123!<br />
          <b>Client login:</b> harry1234boss@gmail.com / Squash123!
        </div>
      </div>
    </div>
  );
...
// ─── NAV ──────────────────────────────────────────────────────────────────────
function Nav({ name, email, role, onLogout }) {
  const isAdmin = email === ADMIN && role === "coach";
  return (
    <nav className="nav">
      <div className="nav-logo">SquashCoach<em>/ {isAdmin ? "Admin" : role === "coach" ? "Coach" : "My Training"}</em></div>
      <div className="nav-r">
        <div className="nav-name"><b>{name}</b>{isAdmin && <span className="pill pill-admin">Admin</span>}{!isAdmin && role === "coach" && <span className="pill pill-coach">Coach</span>}</div>
        <button className="nav-out" onClick={onLogout}>Sign out</button>
      </div>
    </nav>
  );
...
function ProgView({ prog, isCoach, onEdit }) {
  if (!prog) return (
    <div className="no-prog">
      ...
      <div className="no-prog-s">{isCoach ? "Click below to create one." : "Your coach hasn't set your programme yet."}</div>
  ...
  return (
    <div>
      <div className="prog-head">
        <div>
          <div className="prog-fl">Current Block Focus</div>
          <div className="prog-focus">{prog.focus}</div>
        </div>
        <div>
          <div className="prog-wk">{prog.week_label}</div>
          {isCoach && <button className="btn-edit" onClick={onEdit}>Edit Programme</button>}
        </div>
      </div>
          ...
          <div className="row2">
            <div><label className="e-lbl">Type</label>
              <select className="e-inp e-sel" value={day.type} onChange={e => upDay(i, "type", e.target.value)}>
                <option>Squash</option><option>Gym</option><option>Rest</option>
              </select>
            </div>
        ...
        : isCoach
          ? done ? <div className="flash-ok">Response sent ✓</div> : (
            <div className="resp-form">
              <textarea className="resp-ta" placeholder="Write your response to this check-in..." value={resp} onChange={e => setResp(e.target.value)} />
    ...
    <div>
      ...
      <div className="cl-head">
        <div>
          ...
          <div className="cl-sub">{client.level} · {client.goal}</div>
        </div>
  ...
  return (
    <div>
      ...
      {loading ? <div className="empty"><Spin /> Loading clients...</div>
        : clients.length === 0
          ? <div className="empty"><div className="empty-icon">👥</div>No clients yet.</div>
...
function AdminPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setCoaches((cs || []).filter(c => c.email !== ADMIN));
      setClients(cls || []);
    } catch (e) { }
    setLoading(false);
  }, []);
          ...
          {tab === "pending" && (pending.length === 0
            ...
            : <div className="grid-2">{pending.map(c => (
              <div className="card" key={c.id}>
                ...
                <div className="card-meta" style={{ marginBottom: 0 }}>Applied {c.joined_date}</div>
              ...
              <tbody>{clients.map(c => {
                ...
                return (
                  <tr key={c.id}>
                    <td><b>{c.name}</b></td>
                    <td style={{ color: "var(--muted)" }}>{c.email}</td>
                    <td style={{ color: "var(--acc)" }}>{coach?.name || "—"}</td>
        ...
        <div className="two-col">
          ...
          <div className="ck-form-card">
            ...
            <div className="fld">
              ...
              <textarea className="inp ta" style={{ minHeight: 65 }} placeholder="What felt most difficult or limiting?" value={form.hardest} onChange={e => setForm(f => ({ ...f, hardest: e.target.value }))} />
            </div>
            <div className="fld">
...
export default function App() {
  const [user, setUser] = useState(null); // { profile, role, email }

  const login = async (email, password) => {
    // Try coach
    const coaches = await dbGet("coaches", `email=eq.${encodeURIComponent(email)}&password=eq.${encodeURIComponent(password)}`);
    if (coaches?.length) { setUser({ profile: coaches[0], role: "coach", email }); return; }
    // Try client
    const clients = await dbGet("clients", `email=eq.${encodeURIComponent(email)}&password=eq.${encodeURIComponent(password)}`);
    if (clients?.length) { setUser({ profile: clients[0], role: "client", email }); return; }
  ...
  if (user.role === "coach" && user.profile.status === "pending") return <><style>{css}</style><Nav name={user.profile.name} email={user.email} role="coach" onLogout={logout} /><PendingScreen onLogout={logout} /></>;

  const isAdmin = user.email === ADMIN && user.role === "coach";

  return (
    <><style>{css}</style>
    <Nav name={user.profile.name} email={user.email} role={user.role} onLogout={logout} />
    <div className="wrap">
      {isAdmin && <AdminPanel />}
      {!isAdmin && user.role === "coach" && <CoachDash coach={user.profile} />}
      {user.role === "client" && <ClientPortal client={user.profile} />}
    </div></>
  );
}
